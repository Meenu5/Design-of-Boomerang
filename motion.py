...
# Initial Conditions
u_vec = np.array([15.,15.,15.]) # initialize u_vec
omega_vec = np.array([0.,0.,64.*pi/180.]) # initialize omega_vec
u_vec_d = np.zeros(3) # dummy initialisation
Omega = omega_vec[2] # r at t = 0
path_vec = np.array([0.,0.,1.5]) # initial X0,Y0,Z0
phi = 75.*pi/180.
theta = 0.
psi = 0.
Phi0 = 75.*pi/180.
...
J1 = np.matmul(inv(Tj1),inertia_jframe)
J2 = np.matmul(inv(Tj2),inertia_jframe)
...
while steps <= 1 :
    ...
    Cy_A = coefficients.doCy_A(w_vec_1, c, R, S, Omega, alpha_vec_1, Lamda_1, theta_1, beta_1,  length_1, segments)
    Cy_A += coefficients.doCy_A(w_vec_2, c, R, S, Omega, alpha_vec_2, Lamda_2, theta_2, beta_2,  length_2, segments)

    Cz_A = coefficients.doCz_A(w_vec_1, c, R, S, Omega, alpha_vec_1, Lamda_1, theta_1, beta_1,  length_1, segments)
    Cz_A += coefficients.doCz_A(w_vec_2, c, R, S, Omega, alpha_vec_2, Lamda_2, theta_2, beta_2,  length_2, segments)
    ...
    # Moment coefficients
    Cm_x_A = coefficients.doCm_x_A(w_vec_1, c, R, S, Omega, alpha_vec_1, Lamda_1, theta_1, beta_1,  length_1, segments)
    Cm_x_A += coefficients.doCm_x_A(w_vec_2, c, R, S, Omega, alpha_vec_2, Lamda_2, theta_2, beta_2,  length_2, segments)

    Cm_y_A = coefficients.doCm_y_A(w_vec_1, c, R, S, Omega, alpha_vec_1, Lamda_1, theta_1, beta_1,  length_1, segments, x_ac_1)
    Cm_y_A += coefficients.doCm_y_A(w_vec_2, c, R, S, Omega, alpha_vec_2, Lamda_2, theta_2, beta_2,  length_2, segments, x_ac_2)

    Cm_z_A = coefficients.doCm_z_A(w_vec_1, c, R, S, Omega, alpha_vec_1, Lamda_1, theta_1, beta_1,  length_1, segments, x_ac_1)
    Cm_z_A += coefficients.doCm_z_A(w_vec_2, c, R, S, Omega, alpha_vec_2, Lamda_2, theta_2, beta_2,  length_2, segments, x_ac_2)
    ...
    # p angular velocity rate
    t1 = (J1[2][2]-J1[1][1])*omega_vec[1]*omega_vec[2]
    t2 = (J1[0][1]*omega_vec[0] + J1[1][2]*omega_vec[2])*omega_vec[2]
    t3 = -(J1[0][2]*omega_vec[0] + J1[1][2]*omega_vec[1])*omega_vec[1]

    rhs1 = 0.5*rho*((R*Omega)**2)*R*S*Cm_x_A - t1 - t2 - t3
    lhs1 = np.array([J1[0][0],-J1[0][1],-J1[0][2]])

    # q angular velocity rate
    t1 = (J1[0][0]-J1[2][2])*omega_vec[0]*omega_vec[2]
    t2 = (J1[1][2]*omega_vec[1] + J1[0][2]*omega_vec[0])*omega_vec[0]
    t3 = -(J1[0][1]*omega_vec[0] + J1[0][2]*omega_vec[2])*omega_vec[2]

    rhs2 = 0.5*rho*((R*Omega)**2)*R*S*Cm_x_A - t1 - t2 - t3
    lhs2 = np.array([J1[1][1],-J1[1][2],-J1[0][1]])

    # r angular velocity rate
    t1 = (J1[1][1]-J1[0][0])*omega_vec[0]*omega_vec[1]
    t2 = (J1[0][2]*omega_vec[1] + J1[0][1]*omega_vec[1])*omega_vec[1]
    t3 = -(J1[1][2]*omega_vec[0] + J1[0][1]*omega_vec[0])*omega_vec[0]

    rhs3 = 0.5*rho*((R*Omega)**2)*R*S*Cm_x_A - t1 - t2 - t3
    lhs3 = np.array([J1[2][2],-J1[0][2],-J1[1][2]])
